...
export default function AdminAdsScreen() {
  ...
  const validateForm = () => {
    ...
    if (!Number.isFinite(paymentAmount) || paymentAmount <= 0) {
      throw new Error('ยอดชำระต้องเป็นตัวเลขมากกว่า 0');
    }
  ...
  const renderAd = ({ item }) => (
    <View style={[styles.adCard, !item.is_active && styles.adCardInactive]}>
      ...
      <View style={styles.adBody}>
        <View style={styles.adTitleRow}>
          ...
          <Text style={[styles.status, item.is_active ? styles.statusActive : styles.statusInactive]}>
            {item.is_active ? 'เปิดใช้งาน' : 'ปิดใช้งาน'}
          </Text>
        </View>
        <Text style={styles.payment}>ยอดชำระ {Number(item.payment_amount).toLocaleString('th-TH')} บาท</Text>
        <Text style={styles.weightNote}>น้ำหนักการแสดง {Number(item.payment_amount).toLocaleString('th-TH')}</Text>
        ...
        ListHeaderComponent={(
          <View style={styles.headerCard}>
            ...
            <Text style={styles.headerText}>
              ระบบเลือกโฆษณาตามสัดส่วนยอดชำระและหมุนรายการต่อเนื่องบนหน้าแรก
            </Text>
          ...
          <ScrollView contentContainerStyle={styles.modalScroll} keyboardShouldPersistTaps="handled">
            <View style={styles.modalCard}>
              ...
              <TextInput
                ...
                onChangeText={(paymentAmount) => updateForm({ paymentAmount })}
                keyboardType="decimal-pad"
                placeholder="เช่น 1000"
                placeholderTextColor="#999"
              />
              <Text style={styles.hint}>ยอด 1,000 จะแสดงบ่อยกว่ายอด 500 ประมาณสองเท่า</Text>
...
const styles = StyleSheet.create({
  ...
  adActions: { flexDirection: 'row', gap: 9, marginTop: 14 },
